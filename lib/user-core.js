...
/**
 * Genera una contraseña temporal que cumple los requisitos de GHL:
 *   - Mínimo 8 caracteres (generamos ~16-20 para mayor seguridad)
 *   - Al menos una letra mayúscula
 *   - Al menos una letra minúscula
 *   - Al menos un número
 *   - Al menos un carácter especial (!@#$%&*)
 *
 * Formato resultante: Word1word2!99
 * Ejemplo: "Sunfire@37Rock"
 */
function generateTempPassword() {
  ...
  var pickUpper = function () { return upper[crypto.randomInt(upper.length)]; };
  var pickLower = function () { return lower[crypto.randomInt(lower.length)]; };
  var pickSpecial = function () { return specials[crypto.randomInt(specials.length)]; };
  var digits = crypto.randomInt(10, 99);

  // Formato: UpperWord + special + digits + lowerWord
  // Ejemplo: "Rock!47jade" — cumple mayúscula, minúscula, número, especial
  return pickUpper() + pickSpecial() + digits + pickLower();
}
